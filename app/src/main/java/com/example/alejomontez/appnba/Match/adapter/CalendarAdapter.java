package com.example.alejomontez.appnba.Match.adapter;
...
public class CalendarAdapter extends RecyclerView.Adapter {
    ...
    public int getItemViewType(int position) {
        switch (calendarList.get(position).getTipo()) {
            case 0:
        ...
        if (object != null) {
            switch (object.getTipo()) {
                case Calendar.MATCH_TYPE:
                    ...
                    viewHolderMatch.textViewTeamHourMatch.setText(object.getHora());
                    viewHolderMatch.textViewTeamVisitor.setText(object.getVisitante());
                    Picasso.get().load(object.getImg_local()).into(viewHolderMatch.circleImageViewLocal);
                    Picasso.get().load(object.getImg_visitante()).into(viewHolderMatch.circleImageViewVisitor);

                    break;
                case Calendar.TITLE_TYPE:
                    ViewHolderTitle viewHolderTitle = (ViewHolderTitle) holder;
                    viewHolderTitle.textViewCalendarTitle.setText(object.getDia());
                    break;
            }
        }
    }
